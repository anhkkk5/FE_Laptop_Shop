...
import { Minus, Plus, Trash2, Loader2, ShoppingCart, ArrowLeft } from "lucide-react";
    ...
    <div className="container mx-auto max-w-5xl px-4 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{cart.summary.totalItems} sản phẩm</p>
        </div>
        {cart.items.length > 0 && (
          <Button
            variant="outline"
      ...
      {cart.items.length === 0 ? (
        <div className="rounded-xl border p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">Hãy thêm sản phẩm để bắt đầu.</p>
            ...
            {cart.items.map((item) => (
              <div key={item.id} className="rounded-xl border p-4 flex gap-4">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">{formatPrice(Number(item.unitPrice))}</p>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <p className="font-semibold">{formatPrice(Number(item.unitPrice) * item.quantity)}</p>
                      <button
